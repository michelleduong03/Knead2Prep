...
export default function App() {
  ...
  const prepList = rows
    ...
    .filter(Boolean)
    .sort((a, b) => {
      // Sort by urgent first (items with urgent value come first)
      const aHasUrgent = a.urgent && a.urgent.trim() !== "";
      const bHasUrgent = b.urgent && b.urgent.trim() !== "";
      
      if (aHasUrgent && !bHasUrgent) return -1;
      if (!aHasUrgent && bHasUrgent) return 1;
      
      // If both have urgent or both don't, maintain original order
      return 0;
    });
